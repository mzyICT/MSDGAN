...
[x1,fs]=audioread('p232_013__clean.wav');
[x4,fs]=audioread('p232_103__clean.wav');
[x7,fs]=audioread('p257_081__clean.wav');
[x10,fs]=audioread('p257_101__clean.wav');
[x13,fs]=audioread('p257_125__clean.wav');

[x2,fs]=audioread('p232_013__mixed_conditioned0_7dB.wav');
[x5,fs]=audioread('p232_103__mixed_conditioned0_7dB.wav');
[x8,fs]=audioread('p257_081__mixed_conditioned0_6dB.wav');
[x11,fs]=audioread('p257_101__mixed_conditioned0_7dB.wav');
[x14,fs]=audioread('p257_125__mixed_conditioned0_6dB.wav');
...
subplot(5,3,1);myspectrogram(x1,fs);colormap(jet);time=(0:length(x1)-1)/fs;axis([0 max(time*1000) 0 8000]);title('Spectrogram of Clean Signal');ylabel('ID:232-013-Frequency');
...
subplot(5,3,4);myspectrogram(x4,fs);colormap(jet);time=(0:length(x4)-1)/fs;axis([0 max(time*1000) 0 8000]);ylabel('ID:232-103-Frequency');
...
subplot(5,3,7);myspectrogram(x7,fs);colormap(jet);time=(0:length(x7)-1)/fs;axis([0 max(time*1000) 0 8000]);ylabel('ID:257-081-Frequency');
...
subplot(5,3,10);myspectrogram(x10,fs);colormap(jet);time=(0:length(x10)-1)/fs;axis([0 max(time*1000) 0 8000]);ylabel('ID:257-101-Frequency');
...
subplot(5,3,13);myspectrogram(x13,fs);colormap(jet);time=(0:length(x13)-1)/fs;axis([0 max(time*1000) 0 8000]);xlabel('Time');ylabel('ID:257-125-Frequency');
